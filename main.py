...
def init_train_save(config_model, config_trainer, train_data, val_data):
    ...
    if config_model.model_type == 'lstm':
        model = LSTM(vocab_size=vocab_size, output_size=output_size, embedding_size=config_model.embedding_size,
                     hidden_size=config_model.hidden_units, img_feature_size=IMG_FEATURES_LEN,
        ...
        model = BOW(vocab_size=vocab_size, output_size=output_size, embedding_size=config_model.embedding_size,
                    question_len=QUESTION_MAX_LEN, img_feature_size=IMG_FEATURES_LEN, visual_model=config_model.visual_model,
                    pretrained_embeddings=pretrained_embeddings)
    else:
    ...
    trainer.train(model, config_trainer.epochs, config_trainer.lr, config_trainer.verbose, config_trainer.save,
...
# I use this just so I can do config.property
# if I just define config as dict, I need to do config["property"]
parser = argparse.ArgumentParser()
config_model = parser.parse_args()
config_trainer = parser.parse_args()
